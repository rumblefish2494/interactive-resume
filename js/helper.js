...
var HTMLprojectImage = '<img src="%data%" class= "frames">';
...
function initializeMap() {
  ...
  */
  function locationFinder() {
    ...
    for (var school in education.schools) {
      ...
      console.log(education.schools[school].city);
    }

    // iterates through work locations and appends each location to
    // the locations array
    for (var job in work.job) {
      locations.push(work.job[job].location);
      console.log(work.job[job].location);
    }
    //console.log(locations);
    return locations;
  }

  function institutionFinder() {
    var institutions = [];
    institutions.push(bio.name);
    for(var school in education.schools) {
      institutions.push(education.schools[school].name);
      console.log(education.schools[school].name);
    }
    for (var job in work.job) {
      institutions.push(work.job[job].employer);
      console.log(work.job[job].employer);
    }
    //console.log(institutions);
    return institutions;
  }
  institutions = institutionFinder();
  /*
  ...
  */
  ...
  function createMapMarker(placeData) {
    ...
    var place = institutions[counter];
    console.log(institutions[counter]);
    counter = counter + 1;
    ...
    google.maps.event.addListener(marker, 'click', function(event) {
      ...
      var locWindow = new google.maps.InfoWindow({
        content: name//marker["place"]
      });
      locWindow.open(map,marker);
    });
